...
namespace InstaSharper.API.Processors
{
    public class UserProfileProcessor : IUserProfileProcessor
    {
            ...
            string external_url, string email, string phone, string username)
        {
            ...
            {
                if (username == "")
                    return Result.Fail("Use existing or new username, don't leave blank.", false);
                ...
                var request = HttpHelper.GetSignedRequest(HttpMethod.Get, editProfileUri, _deviceInfo, data);
                var response = await _httpRequestProcessor.SendAsync(request);
                var json = await response.Content.ReadAsStringAsync();
                if (response.StatusCode == HttpStatusCode.OK)
                    return Result.Success(true);
