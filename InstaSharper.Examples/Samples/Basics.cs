...
namespace InstaSharper.Examples.Samples
{
    internal class Basics : IDemoSample
    {
        ...
        public async Task DoShow()
        {
            ...
            while (true)
            {
                var teste = await _instaApi.GetUserTimelineFeedAsync(PaginationParameters.MaxPagesToLoad(1));
                var id = teste.Value.NextId;
                Console.WriteLine($"Rodou: {i++}");
            }
            //var userFeed = await _instaApi.GetUserTimelineFeedAsync(PaginationParameters.MaxPagesToLoad(1));
