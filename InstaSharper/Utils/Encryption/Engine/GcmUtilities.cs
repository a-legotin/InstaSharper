...
internal abstract class GcmUtilities
{
    ...
    internal static void AsBytes(uint[] x,
                                 byte[] z)
    {
    ...
    internal static void AsBytes(ulong[] x,
                                 byte[] z)
    {
        Pack.UInt64_To_BE(x, z, 0);
    }
    ...
    internal static void AsUints(byte[] bs,
                                 uint[] output)
    {
        Pack.BE_To_UInt32(bs, 0, output);
    }
    ...
    public static void AsUlongs(byte[] x,
                                ulong[] z)
    {
        Pack.BE_To_UInt64(x, 0, z);
    }

    internal static void Multiply(byte[] x,
                                  byte[] y)
    {
        var t1 = AsUints(x);
        var t2 = AsUints(y);
    ...
    internal static void Multiply(uint[] x,
                                  uint[] y)
    {
    ...
    internal static void Multiply(ulong[] x,
                                  ulong[] y)
    {
        ...
        for (var i = 0; i < 2; ++i)
        {
            ...
            for (var j = 0; j < 64; ++j)
            {
                var m1 = (ulong)(bits >> 63);
                bits <<= 1;
                r10 ^= r00 & m1;
                r11 ^= r01 & m1;

                var m2 = (ulong)((long)(r01 << 63) >> 8);
                r01 = (r01 >> 1) | (r00 << 63);
                r00 = (r00 >> 1) ^ (m2 & E1L);
            }
        }
    ...
    internal static void MultiplyP(uint[] x,
                                   uint[] z)
    {
        var m = (uint)((int)ShiftRight(x, z) >> 8);
        z[0] ^= m & E1;
    }
...
//			}
    ...
    internal static uint ShiftRightN(uint[] x,
                                     int n)
    {
        var b = x[0];
        var nInv = 32 - n;
    ...
    internal static void Xor(byte[] x,
                             byte[] y)
    {
        var i = 0;
        do
    ...
    internal static void Xor(byte[] x,
                             ...
                             int yLen)
    {
        while (--yLen >= 0) x[yLen] ^= y[yOff + yLen];
    }
    ...
    internal static void Xor(byte[] x,
                             ...
                             int len)
    {
        while (--len >= 0) x[xOff + len] ^= y[yOff + len];
    }
    ...
    internal static void Xor(uint[] x,
                             uint[] y)
    {
        x[0] ^= y[0];
        x[1] ^= y[1];
    ...
    internal static void Xor(ulong[] x,
                             ulong[] y)
    {
        x[0] ^= y[0];
        x[1] ^= y[1];
