    ...
    : IBlockCipher
{
    ...
    ///     the block size in bits, one of <see cref="BLOCKSIZE_256" />, <see cref="BLOCKSIZE_512" />,
    ///     <see cref="BLOCKSIZE_1024" /> .
    /// </param>
    ...
    /// </summary>
    /// <param name="forEnc">Initialise for encryption if true, for decryption if false.</param>
    /// <param name="parameters">
    ///     an instance of <see cref="TweakableBlockCipherParameters" /> or <see cref="KeyParameter" /> (to
    ///     use a 0 tweak)
    /// </param>
    public virtual void Init(bool forEnc,
                             ICipherParameters parameters)
    {
        ...
        if (keyBytes != null)
        {
            ...
            for (var i = 0; i < keyWords.Length; i++) keyWords[i] = BytesToWord(keyBytes, i * 8);
        }
        ...
        if (tweakBytes != null)
        {
            ...
            tweakWords = new[] { BytesToWord(tweakBytes, 0), BytesToWord(tweakBytes, 8) };
        }

        Init(forEnc, keyWords, tweakWords);
    }
    ...
    public virtual int ProcessBlock(byte[] inBytes,
                                    int inOff,
                                    byte[] outBytes,
                                    int outOff)
    {
        if (outOff + blocksizeBytes > outBytes.Length) throw new Exception("Output buffer too short");

        if (inOff + blocksizeBytes > inBytes.Length) throw new Exception("Input buffer too short");

        for (var i = 0; i < blocksizeBytes; i += 8) currentBlock[i >> 3] = BytesToWord(inBytes, inOff + i);

        ProcessBlock(currentBlock, currentBlock);
        for (var i = 0; i < blocksizeBytes; i += 8) WordToBytes(currentBlock[i >> 3], outBytes, outOff + i);
    ...
    /// </summary>
    /// <param name="forEnc">the cipher mode.</param>
    ...
    internal void Init(bool forEnc,
                       ulong[] key,
                       ulong[] tweak)
    {
        forEncryption = forEnc;
        if (key != null) SetKey(key);

        if (tweak != null) SetTweak(tweak);
    }
    ...
    internal int ProcessBlock(ulong[] inWords,
                              ulong[] outWords)
    {
        if (kw[blocksizeWords] == 0) throw new InvalidOperationException("Threefish engine not initialised");

        if (inWords.Length != blocksizeWords) throw new Exception("Input buffer too short");

        if (outWords.Length != blocksizeWords) throw new Exception("Output buffer too short");
    ...
    /// </summary>
    internal static ulong BytesToWord(byte[] bytes,
                                      int off)
    {
    ...
    /// </summary>
    internal static void WordToBytes(ulong word,
                                     byte[] bytes,
                                     int off)
    {
        if (off + 8 > bytes.Length)
    ...
    /**
	     ...
	     */
    private static ulong RotlXor(ulong x,
                                 int n,
                                 ulong xor)
    {
    ...
    /**
	     ...
	     */
    private static ulong XorRotr(ulong x,
                                 int n,
                                 ulong xor)
    {
        var xored = x ^ xor;
    ...
    private abstract class ThreefishCipher
    {
        ...
        protected ThreefishCipher(ulong[] kw,
                                  ulong[] t)
        {
            this.kw = kw;
            this.t = t;
        ...
        internal abstract void EncryptBlock(ulong[] block,
                                            ulong[] outWords);

        internal abstract void DecryptBlock(ulong[] block,
                                            ulong[] outWords);
    }
        ...
        : ThreefishCipher
    {
        ...
        public Threefish256Cipher(ulong[] kw,
                                  ulong[] t)
            : base(kw, t)
        {
        }

        internal override void EncryptBlock(ulong[] block,
                                            ulong[] outWords)
        {
            var kw = this.kw;
            var t = this.t;
            ...
            if (kw.Length != 9) throw new ArgumentException();

            if (t.Length != 5) throw new ArgumentException();
        ...
        internal override void DecryptBlock(ulong[] block,
                                            ulong[] state)
        {
            var kw = this.kw;
            var t = this.t;
        ...
        : ThreefishCipher
    {
        ...
        internal Threefish512Cipher(ulong[] kw,
                                    ulong[] t)
            : base(kw, t)
        {
        }
                                            ...
                                            ulong[] outWords)
        {
            ...
            /* Help the JIT avoid index bounds checks */
            if (kw.Length != 17) throw new ArgumentException();
        ...
        : ThreefishCipher
    {
        ...
        public Threefish1024Cipher(ulong[] kw,
                                   ulong[] t)
            : base(kw, t)
        {
        }
                                            ...
                                            ulong[] outWords)
        {
            ...
            /* Help the JIT avoid index bounds checks */
            if (kw.Length != 33) throw new ArgumentException();
