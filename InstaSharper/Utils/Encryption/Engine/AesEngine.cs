    ...
    : IBlockCipher
{
        ...
        ICipherParameters parameters)
    {
        ...
        WorkingKey = GenerateWorkingKey(keyParameter.GetKey(), forEnc);

        forEncryption = forEnc;
        s = Arrays.Clone(forEnc ? S : Si);
    }
    ...
    private static uint Shift(uint r,
                              int shift)
    {
    ...
    */
                                        ...
                                        bool forEnc)
    {
        ...
        for (var i = 0; i <= ROUNDS; ++i) W[i] = new uint[4];
        ...
        switch (KC)
        {
            ...
            case 6:
            {
                ...
                uint rcon1 = 1;
                var u = SubWord(Shift(t5, 8)) ^ rcon1;
                rcon1 <<= 1;
                t0 ^= u;
                W[1][2] = t0;
                t1 ^= t0;
        ...
        if (!forEnc)
            ...
            {
                var w = W[j];
                for (var i = 0; i < 4; i++) w[i] = Inv_Mcol(w[i]);
            }
    ...
    private void DecryptBlock(uint[][] KW)
    {
        ...
        kw = KW[0];
        C0 = Si[r0 & 255] ^ ((uint)s[(r3 >> 8) & 255] << 8) ^ ((uint)s[(r2 >> 16) & 255] << 16) ^
             ((uint)Si[(r1 >> 24) & 255] << 24) ^ kw[0];
        C1 = s[r1 & 255] ^ ((uint)s[(r0 >> 8) & 255] << 8) ^ ((uint)Si[(r3 >> 16) & 255] << 16) ^
             ((uint)s[(r2 >> 24) & 255] << 24) ^ kw[1];
        C2 = s[r2 & 255] ^ ((uint)Si[(r1 >> 8) & 255] << 8) ^ ((uint)Si[(r0 >> 16) & 255] << 16) ^
             ((uint)s[(r3 >> 24) & 255] << 24) ^ kw[2];
        C3 = Si[r3 & 255] ^ ((uint)s[(r2 >> 8) & 255] << 8) ^ ((uint)s[(r1 >> 16) & 255] << 16) ^
             ((uint)s[(r0 >> 24) & 255] << 24) ^ kw[3];
    }
}