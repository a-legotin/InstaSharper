    ...
    : IDigest
{
    ...
    // General parameters:
    private readonly int digestLength = 32; // 1- 32 bytes

    /**
     * Internal state, in the BLAKE2 paper it is called v
     */
    private readonly uint[] internalState = new uint[16];

    private readonly byte[] key;
    private readonly int keyLength; // 0 - 32 bytes for keyed hashing for MAC
    private readonly byte[] personalization;
    private readonly byte[] salt;

    /**
     * Position of last inserted byte
     */
    private int bufferPos; // a value from 0 up to BLOCK_LENGTH_BYTES
                               ...
                               int outOffset)
    {
        ...
        for (var i = 0; i < chainValue.Length && i * 4 < digestLength; i++)
        {
            var bytes = Pack.UInt32_To_LE(chainValue[i]);
    ...
    private void Init()
    {
        if (chainValue == null)
        {
            ...
            chainValue[3] =
                blake2s_IV[3]; // ^ ( (nodeOffset << 32) | (nodeDepth << 16) | (innerHashLength << 24) );
                          ...
                          int messagePos)
    {
        ...
        var m = new uint[16];
        for (var j = 0; j < 16; j++) m[j] = Pack.LE_To_UInt32(message, messagePos + j * 4);
