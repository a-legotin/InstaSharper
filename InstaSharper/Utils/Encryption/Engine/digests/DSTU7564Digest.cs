    ...
    : IDigest, IMemoable
{
    ...
    public virtual void BlockUpdate(byte[] input,
                                    int inOff,
                                    int length)
    {
                               ...
                               int outOff)
    {
        ...
        {
            var inputBytes = bufOff;
            buf[bufOff++] = 0x80;

            var lenPos = blockSize - 12;
            if (bufOff > lenPos)
            {
            ...
            var c = ((inputBlocks & 0xFFFFFFFFUL) * (ulong)blockSize + (uint)inputBytes) << 3;
...

        {
            ...
            for (var col = 0; col < columns; ++col) state[col] ^= tempState1[col];
        }

        var neededColumns = hashSize / 8;
        for (var col = columns - neededColumns; col < columns; ++col)
        {
    ...
    protected virtual void ProcessBlock(byte[] input,
                                        int inOff)
    {
        var pos = inOff;
        for (var col = 0; col < columns; ++col)
        {
            var word = Pack.LE_To_UInt64(input, pos);
            pos += 8;
        ...
        for (var col = 0; col < columns; ++col) state[col] ^= tempState1[col] ^ tempState2[col];
    }
    ...
    private void Q(ulong[] s)
    {
        ...
        {
            /* AddRoundConstantsQ */
            var rc = ((ulong)(((columns - 1) << 4) ^ round) << 56) | 0x00F0F0F0F0F0F0F3UL;
    ...
    private static ulong MixColumn(ulong c)
    {
        ...
        var x1 = ((c & 0x7F7F7F7F7F7F7F7FUL) << 1) ^ (((c & 0x8080808080808080UL) >> 7) * 0x1DUL);
        ulong u, v;
        ...
        v = ((v & 0x3F3F3F3F3F3F3F3FUL) << 2) ^ (((v & 0x8080808080808080UL) >> 6) * 0x1DUL) ^
    ...
    {
        for (var col = 0; col < columns; ++col) s[col] = MixColumn(s[col]);
    }

    private static ulong Rotate(int n,
                                ulong x)
    {
        return (x >> n) | (x << -n);
    }
    ...
    private void SubBytes(ulong[] s)
    {
        for (var i = 0; i < columns; ++i)
        {
            ...
            var t0 = S0[lo & 0xFF];
            var t1 = S1[(lo >> 8) & 0xFF];
            var t2 = S2[(lo >> 16) & 0xFF];
            var t3 = S3[lo >> 24];
            lo = t0 | ((uint)t1 << 8) | ((uint)t2 << 16) | ((uint)t3 << 24);
            var t4 = S0[hi & 0xFF];
            var t5 = S1[(hi >> 8) & 0xFF];
            var t6 = S2[(hi >> 16) & 0xFF];
            var t7 = S3[hi >> 24];
