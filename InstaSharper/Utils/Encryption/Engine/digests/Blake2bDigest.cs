    ...
    : IDigest
{
    ...
    private readonly int keyLength; // 0 - 64 bytes for keyed hashing for MAC
    private readonly byte[] personalization; // new byte[16];

    private readonly byte[] salt; // new byte[16];

    // Position of last inserted byte:
    private int bufferPos; // a value from 0 up to 128

    // called: v
    private ulong[] chainValue; // state vector, in the Blake2b paper it

    private ulong f0; // finalization flag, for last block: ~0L
    // is called: h

    private ulong t0; // holds last significant bits, counter (counts bytes)
    private ulong t1; // counter: Length up to 2^128 are supported
    ...
    /**
         ...
         */
    public virtual void BlockUpdate(byte[] message,
                                    int offset,
                                    int len)
    {
        ...
        int messagePos;
        var blockWiseLastPos = offset + len - BLOCK_LENGTH_BYTES;
                               ...
                               int outOffset)
    {
        ...
        for (var i = 0; i < chainValue.Length && i * 8 < digestLength; i++)
        {
            var bytes = Pack.UInt64_To_LE(chainValue[i]);
                          ...
                          int messagePos)
    {
        ...
        var m = new ulong[16];
        for (var j = 0; j < 16; j++) m[j] = Pack.LE_To_UInt64(message, messagePos + j * 8);
    ...
    private static ulong Rotr64(ulong x,
                                int rot)
    {
        return (x >> rot) | (x << -rot);
    }
