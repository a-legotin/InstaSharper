...
///     <p />
///     This implementation is the basis for <see cref="SkeinDigest" />, implementing the
    ...
    : IMemoable
{
    ...
    static SkeinEngine()
    {
        ...
        InitialState(SKEIN_256, 128, new[]
        {
        ...
        InitialState(SKEIN_256, 160, new[]
        {
        ...
        InitialState(SKEIN_256, 224, new[]
        {
        ...
        InitialState(SKEIN_256, 256, new[]
        {
        ...
        InitialState(SKEIN_512, 128, new[]
        {
        ...
        InitialState(SKEIN_512, 160, new[]
        {
        ...
        InitialState(SKEIN_512, 224, new[]
        {
        ...
        InitialState(SKEIN_512, 384, new[]
        {
        ...
        InitialState(SKEIN_512, 512, new[]
        {
    ...
    public void Reset(IMemoable other)
    {
        ...
        if (BlockSize != s.BlockSize || OutputSize != s.OutputSize)
            throw new Exception("Incompatible parameters in provided SkeinEngine.");

        CopyIn(s);
    }

    private static void InitialState(int blockSize,
                                     int outputSize,
                                     ulong[] state)
    {
    ...
    private static int VariantIdentifier(int blockSizeBytes,
                                         int outputSizeBytes)
    {
    ...
    private void CopyIn(SkeinEngine engine)
    {
        ...
        keyVal = Arrays.Clone(engine.keyVal, keyVal);
    ...
    private static Parameter[] Clone(Parameter[] data,
                                     Parameter[] existing)
    {
        if (data == null) return null;

        if (existing == null || existing.Length != data.Length) existing = new Parameter[data.Length];
    ...
    public void Init(SkeinParameters parameters)
    {
        ...
        if (parameters != null)
        {
            ...
            if (key.Length < 16) throw new ArgumentException("Skein key must be at least 128 bits.");
    ...
    private void CreateInitialState()
    {
        ...
        else
        {
            ...
            // Process key block
            if (keyVal != null) UbiComplete(SkeinParameters.PARAM_TYPE_KEY, keyVal);

            // Process configuration block
            UbiComplete(PARAM_TYPE_CONFIG, new Configuration(OutputSize * 8).Bytes);
        }
    ...
    private void UbiComplete(int type,
                             byte[] value)
    {
        UbiInit(type);
    ...
    {
        if (ubi == null) throw new ArgumentException("Skein engine is not initialised.");
    }
    ...
    {
        CheckInitialised();
        if (outBytes.Length < outOff + OutputSize) throw new Exception("Output buffer is too short to hold output");
        ...
        var blocksRequired = (OutputSize + blockSize - 1) / blockSize;
        for (var i = 0; i < blocksRequired; i++)
        {
            var toWrite = Math.Min(blockSize, OutputSize - i * blockSize);
    ...
    private void Output(ulong outputSequence,
                        byte[] outBytes,
                        int outOff,
                        int outputBytes)
    {
        ...
        {
            var toWrite = Math.Min(8, outputBytes - i * 8);
            if (toWrite == 8)
            {
    ...
    private class UbiTweak
    {
        ...
        /**
             ...
             */
        private ulong[] ubiTweak = new ulong[2];
        ...
        public uint Type
        {
            get => (uint)((ubiTweak[1] >> 56) & 0x3FUL);

            set =>
                // Bits 120..125 = type
                ubiTweak[1] = (ubiTweak[1] & 0xFFFFFFC000000000UL) | ((value & 0x3FUL) << 56);
        }

        public bool First
        {
            get => (ubiTweak[1] & T1_FIRST) != 0;
            set
            {
                if (value)
                    ubiTweak[1] |= T1_FIRST;
        ...
        {
            ubiTweak = Arrays.Clone(tweak.ubiTweak, ubiTweak);
        ...
        public void AdvancePosition(int advance)
        {
            ...
            if (extendedPosition)
            {
                ...
                parts[0] = ubiTweak[0] & 0xFFFFFFFFUL;
                parts[1] = (ubiTweak[0] >> 32) & 0xFFFFFFFFUL;
                parts[2] = ubiTweak[1] & 0xFFFFFFFFUL;
                ...
                ubiTweak[0] = ((parts[1] & 0xFFFFFFFFUL) << 32) | (parts[0] & 0xFFFFFFFFUL);
                ubiTweak[1] = (ubiTweak[1] & 0xFFFFFFFF00000000UL) | (parts[2] & 0xFFFFFFFFUL);
            }
            else
            {
                var position = ubiTweak[0];
                position += (uint)advance;
                ubiTweak[0] = position;
                if (position > LOW_RANGE) extendedPosition = true;
            }
        }
    ...
    private class UBI
    {
        ...
        private readonly UbiTweak tweak = new();
                           ...
                           ulong[] output)
        {
            ...
            while (len > copied)
            {
                ...
                var toCopy = Math.Min(len - copied, currentBlock.Length - currentOffset);
        ...
        private void ProcessBlock(ulong[] output)
        {
            ...
            for (var i = 0; i < message.Length; i++) message[i] = ThreefishEngine.BytesToWord(currentBlock, i * 8);

            engine.threefish.ProcessBlock(message, output);

            for (var i = 0; i < output.Length; i++) output[i] ^= message[i];
        }

        public void DoFinal(ulong[] output)
        {
            // Pad remainder of current block with zeroes
            for (var i = currentOffset; i < currentBlock.Length; i++) currentBlock[i] = 0;
