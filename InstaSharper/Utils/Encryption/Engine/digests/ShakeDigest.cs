    ...
    : KeccakDigest, IXof
{
    ...
    public virtual int DoOutput(byte[] output,
                                int outOff,
                                int outLen)
    {
        if (!squeezing) AbsorbBits(0x0F, 4);
     ...
     * TODO Possible API change to support partial-byte suffixes.
     */
    protected virtual int DoFinal(byte[] output,
                                  int outOff,
                                  int outLen,
                                  byte partialByte,
                                  int partialBits)
    {
        ...
        if (finalBits > 0) AbsorbBits(finalInput, finalBits);
