    ...
    : LongDigest
{
    ...
    private static void UInt64_To_BE(ulong n,
                                     byte[] bs,
                                     int off,
                                     int max)
    {
        if (max > 0)
        {
            ...
            if (max > 4) UInt32_To_BE((uint)n, bs, off + 4, max - 4);
        }
    }

    private static void UInt32_To_BE(uint n,
                                     byte[] bs,
                                     int off,
                                     int max)
    {
        var num = Math.Min(4, max);
        while (--num >= 0)
        {
            var shift = 8 * (3 - num);
    ...
    public override void Reset(IMemoable other)
    {
        ...
        if (digestLength != t.digestLength) throw new Exception("digestLength inappropriate in other");
