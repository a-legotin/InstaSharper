    ...
    : FilterStream
{
        ...
        int length)
    {
        ...
        if ((tag & Asn1Tags.Tagged) != 0) return new Asn1StreamParser(defIn).ReadTaggedObject(isConstructed, tagNo);
    ...
    internal virtual Asn1EncodableVector ReadVector(DefiniteLengthInputStream dIn)
    {
        ...
        Asn1Object o;
        while ((o = subStream.ReadObject()) != null) v.Add(o);
    ...
    public Asn1Object ReadObject()
    {
        ...
        if (length < 0) // indefinite-length method
        {
            ...
            if ((tag & Asn1Tags.Application) != 0) return new BerApplicationSpecificParser(tagNo, sp).ToAsn1Object();

            if ((tag & Asn1Tags.Tagged) != 0) return new BerTaggedObjectParser(true, tagNo, sp).ToAsn1Object();
                                    ...
                                    byte[][] tmpBuffers)
    {
        ...
        if (len >= tmpBuffers.Length) return defIn.ToArray();

        var buf = tmpBuffers[len];
        if (buf == null) buf = tmpBuffers[len] = new byte[len];
